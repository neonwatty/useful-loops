...
Parse the arguments: extract the quoted PROMPT string, the PLAN_FILE path, and an optional `--max N` for max iterations (default: 5).

**Prompt:** `<PROMPT>`
**Plan file:** `<PLAN_FILE>`
**Max iterations:** `<N>`

Repeat Phases 1-4 for up to N iterations. Stop early if Phase 2 finds zero improvements.
...
2. Read `docs/plans/plan-refine-tracking.md` to find the last iteration number. Your iteration is N+1. If no iterations exist yet, you are iteration 1. If the tracking file does not exist, create it:
   ```markdown
...
If zero improvements are found at any severity level, skip Phases 3 and 4. Output exactly:
...
After completing Phase 4, loop back to Phase 1 for the next iteration. Continue until one of the stopping conditions is met:

1. **Iteration count reaches max** — stop and report how many iterations were completed and summarize the cumulative changes.
2. **Zero improvements found** — output `<promise>PLAN_REFINED</promise>` as described in Phase 2.
